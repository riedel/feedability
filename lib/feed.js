 ...
 * 
 * This class uses the sax stream-oriented xml parser to find feed
...
var Feed = function(content, url, mime) {
  ...
  // create sax instance and append listeners but do not parse yet
  this.sax = new sax.parser(true);
  this.addSaxListeners();
}
...
Feed.prototype.parse = function(callback) {
  ...
  this.sax.write(this.content).close(); // what does the second argument do?
  
  // check to see if there was an error during parsing
  var error = this.sax.error;
  if(error) {
    error = 'sax was unable to parse the feed document: ' + error;
  }
 ...
 */
Feed.prototype.addSaxListeners = function() {
  var self = this;
  
  // use closures to keep the scope of the object
  this.sax.onopentag=function(arg){self.onopentag(arg)};
  this.sax.onclosetag=function(arg){self.onclosetag(arg)};
  this.sax.ontext=function(arg){self.ontext(arg)};
  this.sax.onprocessinginstruction=function(arg){self.onprocessinginstruction(arg)};
  this.sax.onopencdata=function(arg){self.onopencdata(arg)};
  this.sax.onclosecdata=function(arg){self.onclosecdata(arg)};
}
 ...
 */
Feed.prototype.onprocessinginstruction = function(inst) {
  var body=inst.body;
  log.debug('sax stream event: xmldecl, encoding: '+body);
  this.header = '<?xml '+body+' ?>\n';
}
 ...
 */
Feed.prototype.onopentag = function(tag) {
  var name=tag.name, attributes=tag.attributes;
  
  log.debug('sax stream event: startelement, name: '+name);
  ...
  // detect item link:
  if(name.toLowerCase() == 'link' && this.item) {
    if(attributes['href']) {
 ...
 */
Feed.prototype.onclosetag = function(name) {
  var tag = null;
  if(this.prev_open) {
    tag = ' />'
    this.prev_open = false;
  ...
  log.debug('sax stream event: endelement, name: '+name+' stream+ '+tag);
  ...
  if(this.item) {
    ...
    if(this.item && this.in_link && name.toLowerCase() == 'link') {
      this.in_link = false;
    }
 ...
 * @param string
 */
Feed.prototype.ontext = function(text) {
  log.debug('sax stream event: text, text length: '+text.length);
  if(!this.cdata_text) {
 ...
 */
Feed.prototype.onopencdata = function() {
  log.debug('sax stream event: startcdata');
  if(this.prev_open) this.stream += '>';
  this.prev_open = false;
 ...
 */
Feed.prototype.onclosecdata = function() {
  log.debug('sax stream event: endcdata');
...
Feed.prototype.createItemExcerpts = function(item) {
  ...
  if(this.mime == 'application/atom+xml') {
    content += '<summary>'+item.excerpts.join('')//.replace(/&/g,'&amp;').replace(/>/g,'&gt;').replace(/</g,'&lt;').replace(/"/g,'&quot;')
              +'</summary>';
  }
  else { // rss
...
Feed.prototype.createItemFullContent = function(item) {
  ...
  if(this.mime == 'application/atom+xml') {
    content += '\n<content type="html">'
            + item.content.replace(/&/g,'&amp;').replace(/>/g,'&gt;').replace(/</g,'&lt;').replace(/"/g,'&quot;')
            +'</content>';
  }
  else { // rss
    if(item.excerpts.length == 0) {
...
Feed.prototype.isItem = function(name) {
  return (name.toLowerCase() == 'item' || name.toLowerCase() == 'entry');
}
