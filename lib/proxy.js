...
var ProxyRequest = function(request, response) {
  ...
  if(match = parsed.href.match(/^\/(.*)?$/)) {
    ...
    if(static_file.indexOf('..') == -1 && func.file_exists(__dirname+'/../static/'+static_file)) {
...
ProxyRequest.prototype.renderStatic = function(filename, vars) {
  ...
  log.info('in dirname: '+file)
  });

  var content = fs.readFileSync(__dirname+'/../static/' + filename, 'utf-8');
